...
""")
    ...
    if st.button("📄 Generate PDF report", type="primary"):
        ...
        pdf.cell(0, 10, f"Stock Report: {sym}", ln=True)
        pdf.set_font("Helvetica", "", 11)
        pdf.cell(0, 8, f"Generated: {datetime.now().strftime('%d %b %Y %H:%M')} IST", ln=True)
        pdf.cell(0, 8, f"Sector: {r.sector} | Framework: {r.frame} | Action: {r.action}", ln=True)
        pdf.ln(3)
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, "Position", ln=True)
        pdf.set_font("Helvetica", "", 11)
        pdf.cell(0, 7, f"Invested: Rs {r.invested:,.0f} | Avg: Rs {r.avg} | Book LTP: Rs {r.ltp}", ln=True)
        pdf.cell(0, 7, f"Qty: {r.qty} (LT {r.lt} / ST {r.st}) | Book value: Rs {r.current:,.0f}", ln=True)
        if live and live.get("price"):
            lp = live["price"]
            live_pnl = lp * r.qty - r.invested
            pdf.cell(0, 7, f"Live: Rs {lp:,.2f} | Live P&L: Rs {live_pnl:,.0f} ({(live_pnl/r.invested)*100:+.1f}%)", ln=True)
        pdf.ln(2)
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, "High / Low / Avg", ln=True)
        pdf.set_font("Helvetica", "", 10)
        for row in rows_hl:
            if row["high"]:
                pdf.cell(0, 6, f"{row['period']}: High Rs {row['high']:,.1f} | Low Rs {row['low']:,.1f} | Avg Rs {row['avg']:,.1f} | From high {row['from_high_pct']:+.1f}%", ln=True)
        pdf.ln(2)
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, "Business & Research", ln=True)
        pdf.set_font("Helvetica", "", 10)
        pdf.multi_cell(0, 5, f"Signal: {r.signal}")
        pdf.multi_cell(0, 5, f"Note: {r.note or '-'}")
        pdf.multi_cell(0, 5, f"Research: {research.get(sym, 'No extended note.')}")
        pdf.ln(2)
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, "Promoter / Institutional", ln=True)
        pdf.set_font("Helvetica", "", 10)
        pdf.multi_cell(0, 5, promoter_notes.get(sym, "Check NSE shareholding pattern for live promoter/FII/DII %."))
        pdf.ln(2)
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, "Sector", ln=True)
        pdf.set_font("Helvetica", "", 10)
        pdf.multi_cell(0, 5, sector_ctx.get(r.sector, "—")[:500])
        pdf.ln(3)
        pdf.set_font("Helvetica", "B", 11)
        pdf.cell(0, 8, "Disclaimer", ln=True)
        pdf.set_font("Helvetica", "", 9)
        pdf.multi_cell(0, 5, "Educational only. Not investment advice. Live data from Yahoo Finance may be delayed. Verify on NSE/BSE. Consult SEBI-registered advisor.")
        buf = io.BytesIO()
        pdf.output(buf)
        st.download_button(
            "Download PDF",
